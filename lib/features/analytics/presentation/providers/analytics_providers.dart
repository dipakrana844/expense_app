...
// Note: DailySnapshot and CategoryInsight classes are now defined in analytics_entity.dart
// Use DailySnapshotEntity and CategoryInsightEntity from domain layer
...
final dailySnapshotProvider = Provider<DailySnapshotEntity>((ref) {
  ...
  return expensesAsync.maybeWhen(
    ...
    orElse: () => DailySnapshotEntity.empty(),
  );
});
        ...
        data: (expenses) {
          ...
          for (var e in currentMonthExpenses) {
            currentTotals[e.category] =
                (currentTotals[e.category] ?? 0) + e.amount;
          ...
          currentTotals.forEach((cat, amt) {
            ...
            insights[cat] = CategoryInsightEntity(
              category: cat,
              amount: amt,
              percentageOfTotal: totalCurrent > 0
                  ? (amt / totalCurrent) * 100
                  : 0,
  ...
  return incomesAsync.when(
    data: (incomes) => expensesAsync.when(
      data: (expenses) {
        ...
        final totalIncome = balanceService.getTotalIncome(
        ...
        final incomeBySource = balanceService.getIncomeSourceBreakdown(
          incomes.cast<IncomeEntity>(),
        );
