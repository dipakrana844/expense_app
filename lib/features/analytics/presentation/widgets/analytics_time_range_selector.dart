...
import '../providers/analytics_providers_optimized.dart';

/// Widget: Analytics Time Range Selector
///
/// Purpose: Allows users to select different time ranges for analytics
/// - Demonstrates the use of family providers with parameterized queries
/// - Updates the analytics data based on selected time range
class AnalyticsTimeRangeSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final currentMonthsBack = ref.watch(analyticsMonthsBackProvider);
  ...
  void _updateTimeRange(WidgetRef ref, int monthsBack) {
    ref.read(analyticsMonthsBackProvider.notifier).state = monthsBack;

    // Refresh the analytics data with the new time range
    final notifier = ref.read(analyticsDataProvider.notifier);
    notifier.refresh();
  }
}
